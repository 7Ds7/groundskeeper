...
function Groundskeeper(options) {
    ...
    // Override default options with the user defined options
...
Groundskeeper.prototype.clean = function (data) {
    ...
    var ast = esprima.parse(data, {
        ...
        source = this.removePragmas(ast.comments, data, this.options.pragmas);

    if (this.options.warn) { return this.warn(source); }

    return falafel(source, { tolerant: true }, function (node) {

        if (this.options.namespace) {
            this.removeNamespace(node);
        }
...
Groundskeeper.prototype.warn = function (ast) {
    var consoles = [];

    return falafel(ast, { tolerant: true }, function () {
        if (node.source() === 'console') {
            consoles.push()
        }
    });
...
Groundskeeper.prototype.removeConsole = function (node) {
    ...
    if (node.type === 'Identifier' && node.source() === 'console') {
        ...
        if (node.parent.type === 'CallExpression') {
            ...
            // Ternary conditionals, return statements and sequences
            if ([
 ...
 * @api private
 */
Groundskeeper.prototype.removeNamespace = function (node) {
...
Groundskeeper.prototype.removeDebugger = function (node) {
    if (node.type === 'DebuggerStatement') { node.update(''); }
};
    ...
    var pragmaMatcher = /^<\/*(\w+)>$/,
        ...
        ranges = comments
            .map(function (comment) {
                if (
                    (!pragmas.length && (pragmaMatcher).test(comment.value)) ||
